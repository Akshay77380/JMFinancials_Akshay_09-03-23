...
class OrderBookController extends GetxController {
  ...
  static var OrderBookLength = 0, pendingLength = 0.obs, executedLength = 0.obs, allLength = 0.obs;
    ...
    try {
      ...
      if (jsons['status'] == false) {
        ...
        for (var i = 0; i < OrderBookList.value.data.length; i++) {
          if (OrderBookList.value.data[i].status == "complete") {
          ...
          } else if(OrderBookList.value.data[i].status == "cancelled" || OrderBookList.value.data[i].status == ""){
            allList.add(OrderBookList.value.data[i]);
          } else if(OrderBookList.value.data[i].status == "open" || OrderBookList.value.data[i].status == "trigger pending" || OrderBookList.value.data[i].status == "recieved" ){
        ...
        allLength.value = allList.length;
