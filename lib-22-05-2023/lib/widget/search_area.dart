...
import '../jmScreens/basketOrder/basketOptionScreen.dart';
...
import '../jmScreens/stockSip/equity_sip_order_screen.dart';
...
class _SearchAreaState extends State<SearchArea> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(
        vsync: this,
        length: Dataconstants.isFromBasketOrder || Dataconstants.isFromSIP
            ? 0
        ...
        Expanded(
          child: TabBarView(
            ...
            children: [
              ...
              if(!Dataconstants.isSip)
                _buildSearchList(widget.dataFO, "Search for stocks to add \n Eg: NIFTY FUT, Nifty CE, Nifty PE, TCS FUT",),
              if (!Dataconstants.isFromAlgo || !Dataconstants.isSip)
                _buildSearchList(
                  widget.dataCurrency,
  ...
  Widget _buildSearchList(List<ScripStaticModel> data, String searchText, [bool isAllUI = false]) {
    ...
    print(data.isEmpty);
    print(isAllUI);
    print(Dataconstants.isSip);
    if(data.isEmpty && Dataconstants.isSip)
      ListView.builder(
          itemCount: data.length,
          itemBuilder: (ctx, index) {
              // int pos = Dataconstants.marketWatchListeners[widget.id].watchList.indexWhere((element) =>
              // element.exch == data[index]. && element.exchCode == data[index].exchCode
              ...
              itemBuilder: (context, i) => ListTile(
                ...
                onTap: () async {
                  ...
                  if (Dataconstants.isSip) {
                    ...
                    setState(() {
                      InAppSelection.searchTabIndex = 1;
                      ScripInfoModel tempModelforSearch =
                      CommonFunction.getScripDataModel(
                          exch: tempModel.exch,
                          exchCode: tempModel.exchCode,
                          getNseBseMap: true);
                      Dataconstants.algoScriptModel = tempModelforSearch;
                      Dataconstants.sipScriptModel = tempModelforSearch;
                    });
                    ...
                    await Dataconstants.iqsClient.sendMarketDepthRequest(tempModel.exch, tempModel.exchCode, true);
                    await Future.delayed(Duration(milliseconds: 100));
                    // (tempModel.exchCategory == ExchCategory.nseFuture)
                    //     ? Navigator.of(context).push(
                    //         MaterialPageRoute(
                    //           builder: (context) => BasketFutureOrderScreen(
                    //               model: tempModel,
                    //               isBuy:
                    //                   InAppSelection.buySellBasketorder == 0
                    //                       ? true
                    //                       : false),
                    //         ),
                    //       )
                    ...
                    Navigator.of(context).push(
                      MaterialPageRoute(
                        builder: (context) => BasketOptionOrderScreen(model: tempModel, isBuy: true),
                      ),
                    );
                    return;
                  }
    ...
    return ListView.builder(
        ...
        itemBuilder: (ctx, index) {
          int pos = Dataconstants.marketWatchListeners[widget.id].watchList.indexWhere((element) =>
            element.exch == data[index].exch && element.exchCode == data[index].exchCode
          );
      ...
      leading: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: InkWell(
              child: Dataconstants.isFromBasketOrder
                  ? Icon(
                      Icons.add,
                      size: 25,
                    )
