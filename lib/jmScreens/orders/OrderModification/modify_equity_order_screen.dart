...
class ModifyEquityOrderScreen extends StatefulWidget {
  ...
  ModifyEquityOrderScreen({@required this.model, @required this.orderType, @required this.isBuy, this.orderModel, this.stream});
  ...
  Widget swipeToAction(Size size, ThemeData theme, double width) {
    return Container(
        ...
        child: TextButton(
          ...
          child: Text('CLICK TO ${widget.orderType == ScripDetailType.positionExit || widget.orderType == ScripDetailType.holdingExit ? 'SQUAREOFF' : widget.isBuy ? 'MODIFY BUY' : 'MODIFY SELL'}'),
        )
        // child: SliderButton(
        //   height: 55,
