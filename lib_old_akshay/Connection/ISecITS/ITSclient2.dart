...
//  Future<Map<String, dynamic>>
  reportFinishedAlgo() async {
    try {
      ...
      Map data = {"ClientCode": Dataconstants.feUserID, "SessionToken": Dataconstants.loginData.data.jwtToken, "InstState": 3};
      //encode Map to JSON
      var body = jsonEncode(data);

      //print("report finished algo api request => $body");
      var response = await http.post(Uri.parse(url), headers: {"Content-Type": "application/json"}, body: body).timeout(timeoutDuration);
      // var response = await CommonFunction.aliceLogging(
      //     link: url,
      //     payload: body,
      //     headers: {"Content-Type": "application/json"},
  ...
  Future<Map<String, dynamic>> stopAllAlgo() async {
    try {
      ...
      //         headers: {"Content-Type": "application/json"}, body: body)
      //     .timeout(timeoutDuration);
      var response = await CommonFunction.aliceLogging(link: url, payload: body, headers: {"Content-Type": "application/json"}, timeoutDuration: timeoutDuration);
      var data2 = jsonDecode(response.body);
  ...
  Future<Map<String, dynamic>> algoLog({int instructionId}) async {
    try {
      ...
      Map data = {"ClientCode": Dataconstants.feUserID, "SessionToken": Dataconstants.loginData.data.jwtToken, "InstID": instructionId};
      //encode Map to JSON
      var body = jsonEncode(data);
  ...
  Future<ApiResponse> createAlgo({
    ...
    try {
      var url = "${BrokerInfo.algoUrl}/instruction/api/algo/create"; // $algoUrl //$algoUrl/instruction //43.242.214.184
      ...
      switch (algoId) {
        ...
        case 2:
          ...
          for (var i = 0; i < dynamicValue.length; i++) {
            jsonData.addAll({dynamicValue[i][0].toString(): dynamicValue[i][1].toString()});
          }
          test = jsonEncode(jsonData);
          break;
        case 3:
