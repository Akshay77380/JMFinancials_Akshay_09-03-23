...
import '../OrderPlacement/equity_order_review_screen.dart';
import '../order_status_details.dart';
import '../order_summary_details.dart';

class ModifyEquityOrderScreen extends StatefulWidget {
  ...
  ModifyEquityOrderScreen({@required this.model, @required this.orderType, @required this.isBuy, this.order ,this.orderModel, this.stream});

  @override
  _ModifyEquityOrderScreenState createState() => _ModifyEquityOrderScreenState();
}

class _ModifyEquityOrderScreenState extends State<ModifyEquityOrderScreen> {
  ...
  List<String> _productItems;
  List<String> _orderValidityItems;
  bool _isSLTPOrder = false, _isCoverOrder = false, _isBracketOrder = false, _isCoverRangeOpen = false, _isInt, isOpen = false, _isQtyDisabled = false, disableSLTP = false;
  ...
  void initState() {
    ...
    if (widget.orderType == ScripDetailType.modify) {
      ...
      _discQtyContoller.text = widget.orderModel.disclosedQty.toString();
      ...
      if (widget.orderType == ScripDetailType.modify && _discQtyContoller.text != '0') isOpen = true;
    } else if (widget.orderType == ScripDetailType.positionExit || widget.orderType == ScripDetailType.holdingExit) {
      ...
      _productItems = widget.orderModel.productType == 'COVER' || widget.orderModel.productType == 'BRACKET' ? [_productType] : ['NRML', 'CNC', 'MIS', 'MTF'];
    ...
    return Column(
      children: [
                    ...
                    children: [
                      ...
                      Obx(() {
                        ...
                        //                       ));
                            ...
                            ? Text(
                            _productType == "NRML" || _productType == "MTF" || _productType == "MIS"
                                ? (int.parse(_qtyContoller.text) * widget.model.close * (double.parse(GetRequiredMarginController.hairCut.value) / 100)).toStringAsFixed(2)
                                : (int.parse(_qtyContoller.text) * widget.model.close).toStringAsFixed(2),
                            style: Utils.fonts(
                              color: Utils.blackColor,
                            ))
                            : Text(
                            _productType == "NRML" || _productType == "MTF" || _productType == "MIS"
                                ? (int.parse(_qtyContoller.text) * double.parse(_limitController.text) * (double.parse(GetRequiredMarginController.hairCut.value) / 100)).toStringAsFixed(2)
                                : (int.parse(_qtyContoller.text) * double.parse(_limitController.text)).toStringAsFixed(2),
                            style: Utils.fonts(
                              color: Utils.blackColor,
                            ));
                      }),
                    ...
                    child: Column(
                      ...
                      children: [
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                NumberField(
                                  ...
                                  isRupeeLogo: false,
                                  defaultValue: 100,
                                  // stoplossOrsquareoff: widget.model.squareoff,
                                  enabled: true,
                                  decimalPosition: 2,
                                  isDisable: false,
                                ),
                                ...
                                NumberField(
                                  ...
                                  hint: 'SL Price',
                                  defaultValue: widget.order != null ? int.parse(widget.order.squareoff) : 0,
                                  isBuy: widget.isBuy,
                                  isRupeeLogo: false,
                                  decimalPosition: 2,
                                  enabled: false,
                        ...
                        NumberField(
                          ...
                          isBuy: widget.isBuy,
                          isRupeeLogo: false,
                          isDisable: widget.orderModel.disclosedQty <= 0 ? true : false,
                          isInteger: true,
                        ),
                        const SizedBox(height: 25),
                        Row(
        ...
        Container(
            ...
            child: TextButton(
              style: ButtonStyle(
                  fixedSize: MaterialStateProperty.all<Size>(Size(size.width * 0.85 - 0.05, 55)),
              ...
              child: Text('${widget.orderType == ScripDetailType.positionExit || widget.orderType == ScripDetailType.holdingExit ? 'SQUAREOFF' : widget.isBuy ? 'MODIFY BUY' : 'MODIFY SELL'}'),
            )
          // child: SliderButton(
          //   height: 55,
...
/* Function to calcutale Required Margin */
  ...
  void processPlace() async {
    ...
    if (widget.orderType == ScripDetailType.positionExit || widget.orderType == ScripDetailType.holdingExit) {
    ...
    if (widget.orderType == ScripDetailType.modify) {
      ...
      // var jsons = {
      //   "exchangeseg": widget.model.exch == "N" ? "NSE" : "BSE",
