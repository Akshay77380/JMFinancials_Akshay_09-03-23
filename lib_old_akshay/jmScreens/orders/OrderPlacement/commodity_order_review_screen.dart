...
class CommodityOrderReviewScreen extends StatefulWidget {
  ...
  @override
  State<CommodityOrderReviewScreen> createState() => _CommodityOrderReviewScreenState();
}

class _CommodityOrderReviewScreenState extends State<CommodityOrderReviewScreen> {
  @override
  Widget build(BuildContext context) {
    ...
    if (widget.orderType != ScripDetailType.cancel) {
      listDetails.add(
        OrderReviewData(
          'ORDER VALUE',
          widget.limitPrice == '0'
              ? '${(int.tryParse(widget.quantity) * widget.model.close).toStringAsFixed(2)}'
              : '${(int.tryParse(widget.quantity) * double.tryParse(widget.limitPrice)).toStringAsFixed(2)}',
        ),
      );
      if (widget.disclosedQty == "0" || widget.disclosedQty == " " || widget.disclosedQty == "") {
      } else {
    ...
    Future<bool> _onWillPop() async {
      setState(() {
        Dataconstants.commodityTransactionButton = true;
      });
      return Dataconstants.commodityTransactionButton;
    }
                ...
                child: Container(
                  ...
                  child: TextButton(
                    style: ButtonStyle(
                        ...
                        backgroundColor: widget.isBuy && Dataconstants.commodityTransactionButton
                            ? MaterialStateProperty.all<Color>(Utils.brightGreenColor)
                            : widget.isBuy && Dataconstants.commodityTransactionButton == false
                                ? MaterialStateProperty.all<Color>(Utils.brightGreenColor.withOpacity(0.5))
                                : widget.isBuy == false && Dataconstants.commodityTransactionButton
                    ...
                    onPressed: Dataconstants.commodityTransactionButton
                        ? () {
                            setState(() {
                              if (Dataconstants.commodityTransactionButton) {
                                Dataconstants.commodityTransactionButton = false;
                              } else {
                                Dataconstants.commodityTransactionButton = true;
                              }
                            });
