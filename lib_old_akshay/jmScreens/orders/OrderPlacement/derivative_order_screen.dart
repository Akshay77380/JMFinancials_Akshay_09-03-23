...
import 'derivative_order_review_screen.dart';

class DerivativeOrderScreen extends StatefulWidget {
  ...
  @override
  State<DerivativeOrderScreen> createState() => _DerivativeOrderScreenState();
}

class _DerivativeOrderScreenState extends State<DerivativeOrderScreen> {
  final GlobalKey<ScaffoldMessengerState> _scaffoldKey = GlobalKey<ScaffoldMessengerState>();
  final _orderTypeController = ValueNotifier<bool>(false), _afterMarketController = ValueNotifier<bool>(false);
  List<String> _productItems = ['NRML', 'MIS'], _orderValidityItems = ['DAY', 'IOC', 'GTC', 'GTD', 'GTT'];
  ...
  void initState() {
    ...
    _orderTypeController.addListener(() {
      ...
      if (_isBracketOrder || _isCoverOrder) {
        setState(() {
          _orderTypeController.value = false;
        });
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
              ...
              child: Column(
                ...
                children: [
                        ...
                        child: Row(
                          children: [
                            SvgPicture.asset(
                              ...
                              'assets/appImages/bidask.svg',color: Theme.of(context).brightness == Brightness.dark ? Utils.whiteColor : Utils.blackColor,
                            ),
                  ...
                  AnimatedSwitcher(
                      ...
                      child: NumberField(
                        doubleDefaultValue: widget.model.strikePrice,
              ...
              InkWell(
                onTap: () async {
                  await Dataconstants.brokerageController.getBrokerage( 
                      ...
                      _productType,
                      DateUtil.getFormattedDate(widget.model.expiryDateString),
                      "NA",
        ...
        //           _numPadController.text.substring(0, cursorPos - 1) +
        //               _numPadController.text
        //                   .substring(cursorPos, _numPadController.text.length);
        //       _numPadController.value = _numPadController.value.copyWith(
  ...
  void processPlace() async {
    ...
    if (_isCoverOrder) {
      if (_coverTriggerController.text.isEmpty) {
        CommonFunction.showSnackBarKey(key: _scaffoldKey, color: Colors.red, context: context, text: 'Trigger price cannnot be empty');
        return;
      }
      if (!widget.isBuy) {
    ...
    Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => DerivativeOrderReviewScreen(
              model: widget.model,
    ...
    //   if (qty > widget.model.freezQty) {
    //     var pendingQty = qty;
    //     var itemCount = (qty / widget.model.freezQty).floor();
