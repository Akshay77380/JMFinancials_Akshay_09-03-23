...
class EquityOrderReviewScreen extends StatefulWidget {
  ...
  @override
  State<EquityOrderReviewScreen> createState() => _EquityOrderReviewScreenState();
}

class _EquityOrderReviewScreenState extends State<EquityOrderReviewScreen> {
  @override
            ...
            Container(
                ...
                child: Container(
                  ...
                  child: TextButton(
                    ...
                    onPressed: Dataconstants.equityTransactionButton
                        ? () {
                            setState(() {
                              if (Dataconstants.equityTransactionButton) {
                                Dataconstants.equityTransactionButton = false;
                              } else {
                                Dataconstants.equityTransactionButton = true;
                              }
                            });
  ...
  void processPlace(BuildContext context) async {
    if (widget.isBracketOrder) {
      ...
      try {
        ...
        if (responseJson["status"] == false) {
          CommonFunction.showBasicToast(responseJson["emsg"]);
          if (responseJson["status"] == false && responseJson["message"] == "Confirmation Required") {
            showDialog(
                context: context,
                builder: (_) => AlertDialog(
                      ...
                      content: IntrinsicHeight(
                        child: Column(
                          children: [Text(responseJson["emsg"])],
                        ),
                      ),
                      actions: <Widget>[
                        TextButton(
                          ...
                          onPressed: () async {
                            if (widget.isBracketOrder) {
                              ...
                              try {
                                var responseJson = json.decode(response.toString());
                                if (responseJson["status"] == false) {
                                  if (responseJson["message"] == "Confirmation Required") {
                                  } else {
                                    CommonFunction.showBasicToast(responseJson["emsg"]);
                                  }
                                } else {
